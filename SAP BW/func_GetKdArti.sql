...
AS RETURN (
  ...
  OWSUmlauf AS (
    ...
    WHERE _Umlauf.Datum = (SELECT MAX(_Umlauf.Datum) FROM OWS.dbo._Umlauf WHERE _Umlauf.Datum <= @Datum)
    GROUP BY _Umlauf.Datum, _Umlauf.ArtGroeID, _Umlauf.VsaID
  )
  SELECT Umlauf.Datum,
  ...
  SELECT OWSUmlauf.Datum,
    ...
    PrListKdArtiID = COALESCE((
      SELECT PrKdArti.ID
      FROM OWS.dbo.KdArti AS PrKdArti
      WHERE PrKdArti.KundenID = Kunden.PrListKundenID
        AND PrKdArti.ArtikelID = KdArti.ArtikelID
        AND PrKdArti.Variante = KdArti.Variante
        AND Kunden.PrListKundenID > 0
    ...
    FirmaWae.ID AS WaeID
  FROM OWS.dbo.KdArti
  JOIN OWS.dbo.Kunden ON KdArti.KundenID = Kunden.ID
  JOIN OWS.dbo.Firma ON Kunden.FirmaID = Firma.ID
  JOIN OWS.dbo.Vsa ON Vsa.KundenID = Kunden.ID
  JOIN OWS.dbo.Artikel ON KdArti.ArtikelID = Artikel.ID
  JOIN OWS.dbo.ArtGroe ON ArtGroe.ArtikelID = Artikel.ID
  JOIN OWS.dbo.KdBer ON KdArti.KdBerID = KdBer.ID
  JOIN OWS.dbo.StandBer ON StandBer.StandKonID = Vsa.StandKonID AND StandBer.BereichID = KdBer.BereichID
  JOIN OWS.dbo.Standort AS Produktion ON StandBer.ProduktionID = Produktion.ID
  JOIN OWS.dbo.Standort AS Expedition ON StandBer.ExpeditionID = Expedition.ID
  JOIN OWS.dbo.ME ON Artikel.MEID = ME.ID
  JOIN OWS.dbo.Wae AS VertragWae ON Kunden.VertragWaeID = VertragWae.ID
  JOIN OWS.dbo.Wae AS RechWae ON Kunden.RechWaeID = RechWae.ID
  JOIN OWS.dbo.Wae AS FirmaWae ON Firma.WaeID = FirmaWae.ID
  JOIN OWSUmlauf ON OWSUmlauf.ArtGroeID = ArtGroe.ID AND OWSUmlauf.VsaID = Vsa.ID
  CROSS APPLY OWS.dbo.advFunc_GetLeasPreisProWo(KdArti.ID) AS LeasPreis
  CROSS APPLY OWS.dbo.advFunc_ConvertExchangeRate(VertragWae.ID, FirmaWae.ID, KdArti.VkPreis, OWSUmlauf.Datum) AS FirmaWae_VKPreis
  CROSS APPLY OWS.dbo.advFunc_ConvertExchangeRate(VertragWae.ID, FirmaWae.ID, KdArti.SonderPreis, OWSUmlauf.Datum) AS FirmaWae_SonderPreis
  CROSS APPLY OWS.dbo.advFunc_ConvertExchangeRate(VertragWae.ID, FirmaWae.ID, KdArti.WaschPreis, OWSUmlauf.Datum) AS FirmaWae_WaschPreis
  CROSS APPLY OWS.dbo.advFunc_ConvertExchangeRate(VertragWae.ID, FirmaWae.ID, LeasPreis.LeasPreisProWo, OWSUmlauf.Datum) AS FirmaWae_LeasPreis
  CROSS APPLY OWS.dbo.advFunc_ConvertExchangeRate(VertragWae.ID, FirmaWae.ID, KdArti.BasisRestwert, OWSUmlauf.Datum) AS FirmaWae_Basisrestwert
  CROSS APPLY OWS.dbo.advFunc_ConvertExchangeRate(VertragWae.ID, FirmaWae.ID, KdArti.GesamtRestwert, OWSUmlauf.Datum) AS FirmaWae_Gesamtrestwert
  CROSS APPLY OWS.dbo.advFunc_ConvertExchangeRate(VertragWae.ID, (SELECT ID FROM OWS.dbo.Wae WHERE Code = N'EUR4'), KdArti.VkPreis, OWSUmlauf.Datum) AS EURWae_VKPreis
  CROSS APPLY OWS.dbo.advFunc_ConvertExchangeRate(VertragWae.ID, (SELECT ID FROM OWS.dbo.Wae WHERE Code = N'EUR4'), KdArti.SonderPreis, OWSUmlauf.Datum) AS EURWae_SonderPreis
  CROSS APPLY OWS.dbo.advFunc_ConvertExchangeRate(VertragWae.ID, (SELECT ID FROM OWS.dbo.Wae WHERE Code = N'EUR4'), KdArti.WaschPreis, OWSUmlauf.Datum) AS EURWae_WaschPreis
  CROSS APPLY OWS.dbo.advFunc_ConvertExchangeRate(VertragWae.ID, (SELECT ID FROM OWS.dbo.Wae WHERE Code = N'EUR4'), LeasPreis.LeasPreisProWo, OWSUmlauf.Datum) AS EURWae_LeasPreis
  CROSS APPLY OWS.dbo.advFunc_ConvertExchangeRate(VertragWae.ID, (SELECT ID FROM OWS.dbo.Wae WHERE Code = N'EUR4'), KdArti.BasisRestwert, OWSUmlauf.Datum) AS EURWae_Basisrestwert
  CROSS APPLY OWS.dbo.advFunc_ConvertExchangeRate(VertragWae.ID, (SELECT ID FROM OWS.dbo.Wae WHERE Code = N'EUR4'), KdArti.GesamtRestwert, OWSUmlauf.Datum) AS EURWae_Gesamtrestwert
  WHERE Artikel.ArtiTypeID = 1
    AND Kunden.AdrArtID = 1
)