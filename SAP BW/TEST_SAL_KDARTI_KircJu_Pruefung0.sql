CREATE OR ALTER VIEW [sapbw].[TEST_SAL_KDARTI_KircJu_Pruefung0]
AS
  WITH Umlauf AS (
    SELECT _Umlauf.Datum, _Umlauf.KdArtiID,_Umlauf.ArtGroeID, _Umlauf.VsaID, SUM(_Umlauf.Umlauf) AS Umlauf
    FROM Salesianer.dbo._Umlauf
    GROUP BY _Umlauf.Datum,_Umlauf.KdArtiID, _Umlauf.ArtGroeID, _Umlauf.VsaID
  )
  SELECT distinct --Umlauf.Datum,
	Dateadd(DAY,7,Umlauf.Datum) as Datum,
    ...
    PrListKdArtiID = COALESCE((
      SELECT TOP 1 PrKdArti.ID
      FROM Salesianer.dbo.KundPrLi
      JOIN Salesianer.dbo.KdArti AS PrKdArti ON PrKdArti.KundenID = KundPrLi.PrListKundenID
      WHERE KundPrLi.KundenID = Kunden.ID
        AND PrKdArti.ArtikelID = KdArti.ArtikelID
        AND PrKdArti.Variante = KdArti.Variante
    ), -1),
    CAST(IIF((SELECT COUNT(*) FROM Salesianer.dbo.KundPrLi WHERE KundPrLi.KundenID = Kunden.ID) > 0, 1, 0) AS bit) AS Preisliste,
    0 as Umlauf,
  ...
  --left JOIN Umlauf Umlauf2 ON Umlauf2.ArtGroeID = ArtGroe.ID AND Umlauf2.VsaID = Vsa.ID and Umlauf2.Datum = Umlauf.datum and Umlauf2.kdartiid = kdarti.id
  ...
  WHERE Artikel.ArtiTypeID = 1
	...
	--and kunden.kdnr =10002809 --and ArtikelNr = 'TS60' and umlauf.Datum = '2022-02-06'
	--and ArtikelNr = '309046'
	and Dateadd(DAY,7,Umlauf.Datum) < GETDATE();