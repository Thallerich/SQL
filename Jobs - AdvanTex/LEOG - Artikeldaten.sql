...
FROM (
  SELECT Kunden.KdNr, Vsa.ID AS VerteilstellenID, Vsa.Bez AS Verteilstellenbezeichnung, Artikel.SuchCode AS Artikelbezeichnung, ROW_NUMBER() OVER (PARTITION BY Vsa.ID ORDER BY Artikel.ArtikelBez ASC) AS SortNumber
  FROM VsaAnf
