...
RAISERROR(N'Setting default value for FahrtID',0,0) WITH NOWAIT;
ALTER TABLE SCANS ADD CONSTRAINT SCANS_FahrtIDDefault DEFAULT -1 FOR FahrtID;
GO

RAISERROR(N'Update all rows where FahrtID is null',0,0) WITH NOWAIT;
UPDATE SCANS SET SCANS.FahrtID = -1 WHERE SCANS.FahrtID IS NULL;
GO

RAISERROR(N'Dropping Index FahrtID on SCANS-Table',0,0) WITH NOWAIT;
DROP INDEX SCANS.FahrtID;
GO

RAISERROR(N'Set FahrtID column to not null',0,0) WITH NOWAIT;
ALTER TABLE SCANS ALTER COLUMN FahrtID int NOT NULL WITH;
GO

RAISERROR(N'Create Index FahrtID on SCANS-Table',0,0) WITH NOWAIT;
