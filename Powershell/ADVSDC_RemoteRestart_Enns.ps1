...
switch($selection) {
  "0" { $Body = "The user $env:USERNAME has executed the remote restart script for SDC Gateways in Enns!" }
  "1" { $Body = "The user $env:USERNAME has executed the remote restart script for SDC Gateway for Metrik 1 (MBK) in Enns!" }
...
if ($errormsg -eq "") {
  ...
  switch($selection) {
    "0" { Write-Host "Neustart für die SDC Gateways für Metrik 1 und Metrik 3 wurde erfolgreich ausgelöst!" -ForegroundColor Green }
    "1" { Write-Host "Neustart für die SDC Gateways für Metrik 1 wurde erfolgreich ausgelöst!" -ForegroundColor Green }
    "2" { Write-Host "Neustart für die SDC Gateways für Metrik 3 wurde erfolgreich ausgelöst!" -ForegroundColor Green }
  }
  ...
  switch($selection) {
    "0" { Write-Host "Fehler beim Auslösen des Neustarts der SDC Gateways für Metrik 1 und Metrik 3!" -ForegroundColor Red }
    "1" { Write-Host "Fehler beim Auslösen des Neustarts der SDC Gateways für Metrik 1!" -ForegroundColor Green }
    "2" { Write-Host "Fehler beim Auslösen des Neustarts der SDC Gateways für Metrik 3!" -ForegroundColor Green }
  }
