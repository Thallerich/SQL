SELECT process.session_id, process.blocking_session_id, process.start_time, session.program_name, session.host_name, session.login_name, session.login_time, db.name, process.status, process.percent_complete, process.command, sqltext.text
