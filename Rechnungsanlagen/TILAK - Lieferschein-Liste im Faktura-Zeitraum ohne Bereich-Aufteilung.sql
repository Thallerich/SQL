...
DECLARE @RechDat date = (SELECT RechDat FROM RechKo WHERE ID = @RechKoID);
DECLARE @MonthFirst date = (SELECT DATEADD(month, DATEDIFF(month, 0, @RechDat), 0));
...
IF @MinDate < @MonthFirst SET @MinDate = @MonthFirst;
