SELECT Kunden.KdNr, Vsa.VsaNr, Vsa.SuchCode AS VsaSuchCode, Vsa.Name1 AS Adresszeile1, Vsa.Name2 AS Adresszeile2, Vsa.Name3 AS Adresszeile3, Vsa.Strasse AS Straße, Vsa.Land, Vsa.PLZ, Vsa.Ort, Vsa.MemoLS, LsKo.LsNr, LsKo.Datum, Touren.Bez AS Tour, Touren.Tour AS TourKurz, TRIM(Mitarbei.Nachname) + IIF(Mitarbei.Vorname <> '', ', ' + TRIM(Mitarbei.Vorname), '') AS Fahrer, Artikel.ArtikelNr, Artikel.ArtikelBez$LAN$ AS Artikelbezeichnung, KdArti.Variante, ArtGroe.Groesse AS Größe, Abteil.Bez AS Kostenstelle, Traeger.Traeger AS Träger, Traeger.Nachname, Traeger.Vorname, EinzHist.Barcode, Scans.[DateTime] AS [Auslese-Zeitpunkt]
FROM Scans
...
ORDER BY Kunden.KdNr, Vsa.VsaNr, LsKo.LsNr, Traeger.Nachname, Artikel.ArtikelNr;