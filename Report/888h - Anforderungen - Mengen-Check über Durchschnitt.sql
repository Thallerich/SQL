...
SELECT Kunden.KdNr, Kunden.SuchCode AS Kunde, Vsa.SuchCode AS VsaStichwort, Vsa.Bez AS VsaBezeichnung, AnfKo.AuftragsNr AS Packzettelnummer, AnfKo.Lieferdatum, Artikel.ArtikelNr, Artikel.ArtikelBez$LAN$ AS ArtikelBez, ArtGroe.Groesse AS Größe, AnfPo.Angefordert, COALESCE(IIF(ArtiStan.Packmenge <= 0, NULL, ArtiStan.Packmenge), Artikel.Packmenge) AS Packmenge, VsaAnf.Durchschnitt, AnfPo.ID AS AnfPoID, Vsa.ID AS VsaID, Artikel.ID AS ArtikelID, AnfPo.ArtGroeID, KdBer.BetreuerID, KdBer.VertreterID, KdBer.ServiceID
