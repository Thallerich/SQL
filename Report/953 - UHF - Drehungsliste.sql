...
SET @sqltext = N'
  SELECT KdGf.KurzBez AS SGF,
    ...
    SUM(IIF(EinzTeil.Status = N''W'' AND EinzTeil.RechPoID > 0, 1, 0)) AS [Schwundmarkiert (verrechnet)],
    SUM(IIF(EinzTeil.Status = N''W'' AND EinzTeil.RechPoID < 0, 1, 0)) AS [Schwundmarkiert (nicht verrechnet)],
