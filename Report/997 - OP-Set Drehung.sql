...
WHERE VsaAnf.VsaID = Vsa.ID
  ...
  AND ArtGru.ID NOT IN (SELECT ID FROM ArtGru WHERE ArtGruBez = 'Instrumente')
  AND Kunden.ID = $ID$
  AND Artikel.ID NOT IN (
    ...
    WHERE NOT EXISTS (
      ...
      WHERE o.Artikel1ID = Artikel.ID
        ...
        AND Artikel.ArtikelNr <> '129899999999' --Trennzeile nicht berücksichtigen
        AND o.ArtikelID = OPSets.ArtikelID
    )
  );

DELETE FROM #TmpOPSetDrehung WHERE ArtikelNr IN ('300400000000', '129900000002');
