...
SELECT AnfKo.Auftragsdatum AS Bestelldatum, AnfKo.Lieferdatum, Kunden.KdNr, Kunden.SuchCode AS Kunde, Vsa.SuchCode AS VsaNr, Vsa.Bez AS Vsa, Artikel.ArtikelNr, Artikel.ArtikelBez$LAN$ AS Artikelbezeichnung, AnfPo.Angefordert AS Bestellt, 0 AS Geliefert, 0 AS Verrechnet, CONVERT(money, 0) AS Einzelpreis, AnfKo.LsKoID, AnfPo.KdArtiID, AnfPo.VpsKoID, AnfPo.ArtGroeID, AnfPo.LsKoGruID
INTO #TmpAnfData
...
SELECT Bestelldatum, Lieferdatum, KdNr, Kunde, VsaNr, Vsa, ArtikelNr, Artikelbezeichnung, SUM(Bestellt) AS Bestellt, SUM(Geliefert) AS Geliefert, SUM(Verrechnet) AS Verrechnet, Einzelpreis
FROM #TmpAnfData AnfData
GROUP BY Bestelldatum, Lieferdatum, KdNr, Kunde, VsaNr, Vsa, ArtikelNr, Artikelbezeichnung, Einzelpreis
