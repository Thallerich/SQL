...
SELECT KdGf.KurzBez AS Geschäftsbereich, Kunden.KdNr, Kunden.SuchCode AS Kunde, Kundenstatus.StatusBez AS Kundenstatus, Vsa.VsaNr, Vsa.SuchCode AS [VSA-Stichwort], Vsa.Bez AS [VSA-Bezeichnung], VsaStatus.StatusBez AS [VSA-Status], VSA.Name1, VSA.Name2, Vsa.GebaeudeBez AS Gebäude, Abteil.Abteilung AS [Stamm-Kostenstelle], Abteil.Bez AS [Stamm-Kostenstellenbezeichnung], Traeger.Traeger AS [Träger-Nr], Traeger.PersNr AS Personalnummer, Traeger.Nachname, Traeger.Vorname, Traeger.Titel, Traegerstatus.StatusBez AS Trägerstatus, IIF(Traeger.BerufsgrKdArtiID < 0, NULL, Berufsgruppe.VariantBez) AS Berufsgruppe, Traeger.SchrankInfo, Artikel.ArtikelNr, Artikel.ArtikelBez$LAN$ AS Artikelbezeichnung, ArtGroe.Groesse AS Größe, EinzHist.Barcode, TeileStatus.StatusBez AS [Status Teil], EinzHist.Indienst as [Erstwoche], EinzHist.Abmeldung, RestwertAktuell.RestwertInfo AS Restwert, RestwertAktuell.Alterinfo AS [Alter in Wochen], Standort.Suchcode AS Expedition, Einsatz.EinsatzBez$LAN$ AS Ausdienstgrund, Weggrund.WeggrundBez$LAN$ AS Schrottgrund, [Service].[Name] AS Kundenservice, Betreuer.[Name] AS Kundenbetreuer, Vertreter.[Name] AS Vertrieb, KdArti.LeasPreis AS Mietpreis, KdArti.ID AS KdArtiID
FROM EinzTeil
...
JOIN Mitarbei AS [Service] ON VsaBer.ServiceID = [Service].ID
