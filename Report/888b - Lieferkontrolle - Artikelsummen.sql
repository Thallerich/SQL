...
SELECT LSK.Standortkonfiguration, IIF($5$ = 0, 0, LSK.KdNr) AS KdNr, IIF($5$ = 0, '', LSK.Kunde) AS Kunde, LSK.Datum, LSK.ArtikelNr, LSK.ArtikelBez, SUM(IIF(LSK.Sonderfahrt = $FALSE$, LSK.Angefordert, 0)) AS Angefordert, SUM(IIF(LSK.Sonderfahrt = $TRUE$, LSK.Angefordert, 0)) AS [Angefordert Sonderlieferung], SUM(LSK.Liefermenge) AS Liefermenge, SUM(LSK.AnzahlChips) AS [Anzahl Chips], 0 AS Abweichung, CONVERT(float, 0) AS AbweichungProzent
INTO #TmpFinal
FROM #TmpLsKontrolle888b LSK
GROUP BY KdNr, Kunde, LSK.Standortkonfiguration, LSK.Datum, LSK.ArtikelNr, LSK.ArtikelBez;
