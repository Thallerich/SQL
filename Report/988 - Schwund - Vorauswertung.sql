...
INTO #TmpSchwund
FROM OPTeile, Vsa, Kunden, ArtGroe, KdArti, Artikel, Bereich, (SELECT Status.Status, Status.StatusBez$LAN$ AS Bez FROM Status WHERE Status.Tabelle = 'VSA') AS Status
...
GROUP BY Kunden.KdNr, Kunden.SuchCode, Vsa.VsaNr, Status.Bez, Vsa.SuchCode, Vsa.Bez, Bereich.BereichBez$LAN$, Artikel.ArtikelNr, Artikel.ArtikelBez$LAN$, Artikel.EKPreis, Vsa.ID, Artikel.ID, ArtGroe.ID;
