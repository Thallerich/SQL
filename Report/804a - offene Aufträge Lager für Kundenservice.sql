...
INSERT INTO #Result804a (ServiceMA, BetreuerMA, KdNr, Kunde, Holding, Nachname, Vorname, Trägerstatus, Barcode, Teilestatus, ArtikelNr, Artikelbezeichnung, Größe, Anlage, Entnahmeliste, EntnahmeAnlage, EntnahmeDruck, EntnhameZeit, EntnahmePatch, Einsatzgrund)
SELECT ServiceMA.Name AS ServiceMA, BetreuerMA.Name AS BetreuerMA, Kunden.KdNr, Kunden.SuchCode AS Kunde, Holding.Holding, Traeger.Nachname, Traeger.Vorname, Traegerstatus.StatusBez AS Trägerstatus, EinzHist.Barcode, Teilestatus.StatusBez AS Teilestatus, Artikel.ArtikelNr, Artikel.ArtikelBez$LAN$ AS Artikelbezeichnung, ArtGroe.Groesse AS Größe, EinzHist.Anlage_ AS Anlage, EntnKo.ID AS Entnahmeliste, EntnKo.Anlage_ AS EntnahmeAnlage, EntnKo.DruckDatum AS EntnahmeDruck, EntnahmeZeit = (
...
SELECT ServiceMA AS [Kundenservice-Mitarbeiter], BetreuerMA AS Betreuer, KdNr, Kunde, Holding, Nachname, Vorname, Trägerstatus, Barcode, Teilestatus, ArtikelNr, Artikelbezeichnung, Größe, Anlage AS [Teil angelegt am], Entnahmeliste AS EntnahmelistenNr, EntnahmeAnlage AS [Entnahmeliste angelegt am], EntnahmeDruck AS [Druckdatum Entnahmeliste], EntnhameZeit AS [Entnahme-Datum], EntnahmePatch AS [Patchdatum Entnahmeliste], Einsatzgrund
FROM #Result804a
