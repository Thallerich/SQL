...
SELECT KdGf.KurzBez AS SGF, EinzTeil.Code AS Barcode, Artikel.ArtikelNr, Artikel.ArtikelBez$LAN$ AS Artikelbezeichnung, Artikel.EKPreis, ErstWoche = (SELECT [Week].Woche FROM [Week] WHERE DATEADD(day, EinzTeil.AnzTageImLager, EinzTeil.ErstDatum) BETWEEN [Week].VonDat AND [Week].BisDat), Eingangsscan.LastScanIn AS [letzter Eingangsscan], EinzTeil.WegDatum, WegGrund.WegGrundBez$LAN$ AS WegGrund, EinzTeil.RuecklaufG AS [Anzahl Wäschen], Kunden.KdNr, Kunden.SuchCode AS Kunde, Vsa.VsaNr, Vsa.Bez AS [Vsa-Bezeichnung]
FROM EinzTeil
