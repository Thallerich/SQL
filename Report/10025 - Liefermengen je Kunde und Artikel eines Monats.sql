...
FROM (
  SELECT Kunden.KdNr, Kunden.SuchCode AS Kunde, Artikel.ArtikelNr, Artikel.ArtikelBez$LAN$ AS Artikelbezeichnung, ArtGroe.Groesse AS [Größe], DATEPART(day, LsKo.Datum) AS Tag, ROUND(SUM(LsPo.Menge), 0) AS Liefermenge
  FROM LsPo
  ...
  GROUP BY Kunden.KdNr, Kunden.SuchCode, Artikel.ArtikelNr, Artikel.ArtikelBez$LAN$, ArtGroe.Groesse, DATEPART(day, LsKo.Datum)
) AS PivoData
