...
AS (
  ...
  FROM OPScans
  WHERE CAST(OPScans.Zeitpunkt AS date) = CAST(GETDATE() AS date)
    AND OPScans.AnlageUserID_ = (SELECT MitarbeiID FROM #AdvSession)
...
FROM (
  SELECT ISNULL(Mitarbei.MitarbeiUser, N'Summe') AS Benutzername, Mitarbei.Name AS Mitarbeiter, ISNULL(DATEPART(hour, DistinctScans.Zeitpunkt), 99) AS Stunde, COUNT(DistinctScans.OPTeileID) AS [Anzahl Teile]
  FROM DistinctScans
