...
FROM AnfPo, AnfKo, Vsa, Kunden, ABC, KdArti, Artikel, VsaAnf, Standort AS Expedition, (
    SELECT DISTINCT VsaTour.VsaID, VsaTour.KdBerID, Touren.ExpeditionID
    FROM VsaTour
...
WHERE AnfPo.AnfKoID = AnfKo.ID
  ...
  AND TourenExpedition.VsaID = Vsa.ID
  AND TourenExpedition.KdBerID = KdArti.KdBerID
  AND TourenExpedition.ExpeditionID = Expedition.ID
