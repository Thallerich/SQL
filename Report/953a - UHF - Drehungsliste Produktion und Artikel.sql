...
DECLARE @stark int, @schwach int, @kaum int;

SET @stark = $1$;
SET @schwach = $2$;
SET @kaum = $3$;

DROP TABLE IF EXISTS #Artikelauswahl, #Produktionsauswahl;

SELECT Standort.ID
INTO #Produktionsauswahl
FROM Standort
...
SET @sqltext = N'
  ...
  WHERE Artikel.ID IN (SELECT ID FROM #Artikelauswahl)
