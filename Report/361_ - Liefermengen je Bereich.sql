...
WHERE LsPo.LsKoID = LsKo.ID
  ...
  AND Artikel.ID > 0
GROUP BY KdGf.KurzBez, KdGf.Bez, Kunden.KdNr, Kunden.SuchCode, CONVERT(char(4), DATEPART(year, LsKo.Datum)) + '/' + IIF(DATEPART(month, LsKo.Datum) < 10, '0' + CONVERT(char(1), DATEPART(month, LsKo.Datum)), CONVERT(char(2), DATEPART(month, LsKo.Datum))), Bereich.BereichBez$LAN$, Artikel.ArtikelNr, Artikel.ArtikelBez$LAN$
