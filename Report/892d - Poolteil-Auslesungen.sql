...
SET @sqltext = N'
SELECT [Zone].ZonenCode AS Vertriebszone, KdGf.KurzBez AS Geschäftsbereich, Holding.Holding, Kunden.KdNr, Kunden.SuchCode AS Kunde, Vsa.VsaNr, Vsa.Bez AS [VSA-Bezeichnung], Artikel.ArtikelNr, Artikel.ArtikelBez$LAN$ AS Artikelbezeichnung, ArtGroe.Groesse AS Größe, LsKo.LsNr, LsKo.Datum AS Lieferdatum, EinzTeil.Code, EinzTeil.Code2, Scans.[DateTime] AS [Zeitpunkt Auslesung], ZielNr.ZielNrBez$LAN$ AS [Ort Auslesung], Standort.Bez AS Produktion
FROM Scans
...
WHERE Scans.[DateTime] BETWEEN @startdate AND @enddate
...
INSERT INTO #Pool892d (Vertriebszone, Geschäftsbereich, Holding, KdNr, Kunde, VsaNr, [VSA-Bezeichnung], ArtikelNr, Artikelbezeichnung, Größe, LsNr, Lieferdatum, Code, Code2, [Zeitpunkt Auslesung], [Ort Auslesung], Produktion)
