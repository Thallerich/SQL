...
SELECT Artikel.ArtikelNr, Artikel.ArtikelBez$LAN$ AS Artikelbezeichnung, Artikelstatus.StatusBez AS Artikelstatus, Standort.SuchCode AS Produktionsstandort, FaltProg.Programm AS Faltprogramm, FaltProg.FaltProgBez AS Faltprogrammbezeichnung, FinishPr.Programm AS Finishprogramm, FinishPrBez AS Finishprogrammbezeichnung
FROM ArtiStan
...
WHERE Standort.ID = @Produktion
  AND (($2$ = 1 AND (FaltProg.ID < 0 OR FinishPr.ID < 0)) OR ($2$ = 0))
  AND Artikel.ID > 0
