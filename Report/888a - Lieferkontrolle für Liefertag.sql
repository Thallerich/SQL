...
SELECT Kunden.KdNr, Kunden.SuchCode AS Kunde, Vsa.SuchCode AS VsaStichwort, Vsa.Bez AS VsaBezeichnung, CONVERT(varchar(10), NULL) AS LsNr, CONVERT(date, NULL) AS Datum, AnfKo.AuftragsNr AS Packzettelnummer, Artikel.ArtikelNr, Artikel.ArtikelBez$LAN$ AS ArtikelBez, IIF(DATEDIFF(minute, AnfPo.Anlage_, AnfPo.BestaetZeitpunkt) = 0, 0, AnfPo.Angefordert) AS Angefordert, 0 AS Liefermenge, 0 AS AnzahlChips, 0 AS Abweichung, CONVERT(numeric(7,2), 0) AS AbweichungProzent, IIF(KdArti.Vorlaeufig = 1 AND KdArti.AnlageUserID_ IN (SELECT Mitarbei.ID FROM Mitarbei WHERE Mitarbei.UserName IN ('TAGJOB', 'CITJOB')), 1, 0) AS NichtKdArti, 0 AS LsIsOK, AnfPo.KdArtiID, AnfKo.LsKoID, AnfPo.ID AS AnfPoID, Vsa.ID AS VsaID
INTO #TmpLsKontrolle888a
FROM AnfPo, AnfKo, Vsa, Kunden, KdArti, Artikel
...
SELECT KdNr, Kunde, VsaStichwort, VsaBezeichnung, LsNr, Datum, Packzettelnummer, ArtikelNr, ArtikelBez, Angefordert, Liefermenge, AnzahlChips, Abweichung, FORMAT(AbweichungProzent, 'P2', 'de-AT') AS AbweichungProzent, NichtKdArti, LsIsOK, KdArtiID, LsKoID, AnfPoID, VsaID
