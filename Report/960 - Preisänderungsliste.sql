SELECT Kunden.KdNr, Kunden.SuchCode AS Kunde, Artikel.ArtikelNr, Artikel.ArtikelBez AS Artikelbezeichnung, KdArti.Variante, Status.StatusBez AS [Kundenartikel-Status], PrArchiv.Datum AS [Änderung effektiv ab], KdArti.WaschPreis AS [Bearbeitung aktuell], KdArti.LeasingPreis AS [Leasing aktuell], KdArti.PeriodenPreis AS [Periodenpreis aktuell], ISNULL(PeKo.Bez, N'') AS Preiserhöhung, ISNULL(Mitarbei.Name, N'') AS [PE-Durchführungs-Mitarbeiter], KdArti.ID AS KdArtiID
FROM PrArchiv
