...
SELECT Lagerart.Lagerart, Lagerart.LagerartBez AS Lagerartbezeichnung, BKo.BestNr AS [Bestellung Nr.], BKo.Datum AS Bestelldatum, Bestellstatus.StatusBez AS [Status der Bestellung], BPo.Pos AS Positionsnummer, Artikel.ArtikelNr, Artikel.ArtikelBez AS Artikelbezeichnung, ArtGroe.Groesse AS Größe, BPo.Menge AS [bestellte Menge], BPo.LiefMenge AS [bereits geliefert], IIF(BPo.LiefMenge > BPo.Menge, 0, BPo.Menge - BPo.LiefMenge) AS [noch offen], LiefAbKo.ABNr, LiefAbKo.Datum AS [AB-Datum], LiefAbPo.Menge AS [bestätigte Menge], LiefAbPo.Termin AS [Liefertermin lt. AB], BKo.ID AS BKoID
FROM BPo
