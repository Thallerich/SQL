SELECT EinzHist.Barcode, Artikel.ArtikelNr, Artikel.ArtikelBez$LAN$ AS Artikelbezeichnung, ArtGroe.Groesse AS Größe, EinzHist.Anlage_ AS [Zeitpunkt Einbuchung], Einzteil.Erstdatum as [Erstdatum], EinzTeil.RuecklaufG AS [Anzahl Wäschen], Lagerart.LagerartBez$LAN$ AS Lagerart, Lagerart.Neuwertig, Lager.Bez AS Lagerstandort, Lagerort.Lagerort, Kunden.KdNr AS [KdNr letzter Kunde], Kunden.SuchCode AS [letzter Kunde], Restwert.Restwertinfo AS [Restwert in Lagerwährung], Firma.WaeID AS [Restwert in Lagerwährung_WaeID] , EinzTeil.AlterInfo AS [Alter in Wochen], CAST(IIF(EXISTS(SELECT TeilAppl.ID FROM TeilAppl WHERE TeilAppl.EinzHistID = EinzHist.ID), 1, 0) AS bit) [mit Applikation]
FROM EinzTeil
JOIN EinzHist ON EinzTeil.CurrEinzHistID = EinzHist.ID
CROSS APPLY advfunc_GetRestwertIgnoreAusdRestW(EinzHist.ID, (SELECT [Week].Woche FROM [Week] WHERE CAST(GETDATE() AS date) BETWEEN [Week].VonDat AND [Week].BisDat), 1) AS Restwert
