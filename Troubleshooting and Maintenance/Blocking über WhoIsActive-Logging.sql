SELECT TOP 100 DENSE_RANK() OVER (ORDER BY WhoIsActive.collection_time) AS GroupNr, WhoIsActive.collection_time, WhoIsActive.session_id, WhoIsActive.blocking_session_id, WhoIsActive.sql_text, WhoIsActive.sql_command, WhoIsActive.wait_info, REPLACE(SUBSTRING(WhoIsActive.wait_info, 2, CHARINDEX(N')', WhoIsActive.wait_info, 1) - 2), N'ms', N'') AS wait_time, WhoIsActive.login_name, WhoIsActive.status, WhoIsActive.tran_start_time, WhoIsActive.host_name, WhoIsActive.database_name, WhoIsActive.program_name, WhoIsActive.start_time, WhoIsActive.additional_info
