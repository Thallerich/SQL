...
FROM (
  ...
  FULL OUTER JOIN (
    SELECT LsKo.VsaID, LsPo.KdArtiID, Kunden.KdNr, Kunden.Debitor, KdGf.KurzBez AS SGF, Artikel.ArtikelNr, Artikel.ArtikelBez, LsPo.EPreis, SUM(LsPo.Menge) AS Liefermenge, Standort.SuchCode AS Produzent, Standort.FibuNr
    ...
    WHERE LsKo.Datum BETWEEN N'2018-04-01' AND N'2018-04-30'
      ...
      AND Artikel.ID > 0
      AND KdGf.KurzBez <> N'ÖS'
    GROUP BY LsKo.VsaID, LsPo.KdArtiID, Kunden.KdNr, Kunden.Debitor, KdGf.KurzBez, Artikel.ArtikelNr, Artikel.ArtikelBez, LsPo.EPreis, Standort.SuchCode, Standort.FibuNr
