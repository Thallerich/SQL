...
SELECT Kunden.KdNr, Kunden.Debitor, KdGf.KurzBez AS SGF, Bereich.BereichBez AS Produktbereich, ArtGru.ArtGruBez AS Artikelgruppe, IIF(Artikel.ID < 0, N'', Artikel.ArtikelNr) AS ArtikelNr, ISNULL(Artikel.ArtikelBez, N'') AS Artikelbezeichnung, SUM(FibuDet.Menge) AS VerrechMenge, FibuDet.EPreis, SUM(FibuDet.GPreis) AS UmsatzNetto, Konten.Konto AS Erlöskonto,
  FibuNrVertrieb = 
    CASE @FirmaID
      WHEN 5001 THEN N'93 '
      WHEN 5260 THEN N'90 '
...
JOIN ArtGru WITH (NOLOCK) ON Artikel.ArtGruID = ArtGru.ID
...
GROUP BY Kunden.KdNr, Kunden.Debitor, KdGf.KurzBez, Bereich.BereichBez, ArtGru.ArtGruBez, IIF(Artikel.ID < 0, N'', Artikel.ArtikelNr), Artikel.ArtikelBez, FibuDet.EPreis, Konten.Konto, 
  CASE @FirmaID
    WHEN 5001 THEN N'93 '
    WHEN 5260 THEN N'90 '
    WHEN 5256 THEN N'895'
...
SELECT LsKo.VsaID, LsPo.KdArtiID, KdBer.BereichID, KdGf.ID AS KdGfID, Kunden.MwStID, Artikel.ArtGruID, Bereich.BereichBez AS Produktbereich, ArtGru.ArtGruBez AS Artikelgruppe, Kunden.KdNr, Kunden.Debitor, KdGf.KurzBez AS SGF, Artikel.ArtikelNr, Artikel.ArtikelBez, SUM(LsPo.Menge) AS Liefermenge, Standort.SuchCode AS Produzent,
  FibuNr = 
  CASE
...
WHERE LsKo.Datum BETWEEN @DatumVon AND @DatumBis
  ...
  AND Artikel.ID > 0
  AND KdGf.KurzBez IN (N'MED', N'GAST', N'JOB')
GROUP BY LsKo.VsaID, LsPo.KdArtiID, KdBer.BereichID, KdGf.ID, Kunden.MwStID, Artikel.ArtGruID, Bereich.BereichBez, ArtGru.ArtGruBez, Kunden.KdNr, Kunden.Debitor, KdGf.KurzBez, Artikel.ArtikelNr, Artikel.ArtikelBez, Standort.SuchCode,
 CASE
    WHEN Standort.FibuNr = N'663' AND @DatumBis < N'2019-08-01' THEN N'673'
    WHEN Standort.FibuNr = N'666' AND @DatumBis < N'2019-08-01' THEN N'676'
    ELSE Standort.FibuNr
...
SELECT KdNr, Debitor, SGF AS Vertrieb, Produktbereich, Artikelgruppe, ArtikelNr, Artikelbezeichnung, SUM(VerrechMenge) AS [verrechnete Menge], SUM(UmsatzNetto) AS UmsatzNetto, Erlöskonto, RTRIM(FibuNrVertrieb) + KostenträgerVertrieb [Kostenträger Vertrieb], Produzent, RTRIM(FibuNr) + Kostenträger AS [Kostenträger Produzent], Verrechnungsart = 
  CASE
...
GROUP BY KdNr, Debitor, SGF, Produktbereich, Artikelgruppe, ArtikelNr, Artikelbezeichnung, Erlöskonto, RTRIM(FibuNrVertrieb) + KostenträgerVertrieb, Produzent, RTRIM(FibuNr) + Kostenträger,
  CASE
    WHEN IsLeasing = 1 AND IsStueck = 0 THEN N'Pauschal'
    WHEN IsStueck = 1  AND IsLeasing = 0 THEN N'Stück'
    WHEN IsLeasing = 1 AND IsStueck = 1 THEN N'Splitting'
...
SELECT KdNr, Debitor, SGF AS Vertrieb, Produktbereich, Artikelgruppe, ArtikelNr, Artikelbezeichnung, SUM(Liefermenge) AS [Liefermenge], RTRIM(FibuNrVertrieb) + KostenträgerVertrieb [Kostenträger Vertrieb], Produzent, RTRIM(FibuNr) + Kostenträger AS [Kostenträger Produzent], Verrechnungsart = 
  CASE
    WHEN IsLeasing = 1 AND IsStueck = 0 THEN N'Pauschal'
    WHEN IsStueck = 1  AND IsLeasing = 0 THEN N'Stück'
    WHEN IsLeasing = 1 AND IsStueck = 1 THEN N'Splitting'
