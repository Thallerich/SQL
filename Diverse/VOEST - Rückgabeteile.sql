...
SELECT Kunden.KdNr, Kunden.SuchCode AS Kunde, Teile.Barcode, Teilestatus.StatusBez AS [Status des Teils], Traeger.Traeger AS TrägerNr, Traeger.Vorname, Traeger.Nachname, Traegerstatus.StatusBez AS [Status des Trägers], Abteil.Abteilung AS Kostenstele, Abteil.Bez AS Kostenstellenbezeichnung, Vsa.GebaeudeBez AS Abteilung, Artikel.ArtikelNr, Artikel.ArtikelBez AS Artikelbezeichnung, KdArti.Variante, Teile.AbmeldDat AS [Datum Abmeldung], LeasProWo.LeasPreisProWo AS [Leasing pro Woche]
FROM Teile
