...
SELECT Standort.Bez AS Lagerstandort, LagerArt.LagerartBez AS Lagerart, TeileLag.Barcode, Artikel.ArtikelNr, Artikel.ArtikelBez AS Artikelbezeichnung, ArtGroe.Groesse AS [Größe], LagerteilStatus.StatusBez AS [Status Lager-Teil], FORMAT(TeileLag.Restwert, 'C', 'de-AT') AS [Restwert bei Einlagerung], TeileLag.AnzWaschen AS [Anzahl Wäschen], FORMAT(TeileLag.ErstDatum, 'd', 'de-at') AS [Erste Auslieferung zu erstem Kunden], TeileLag.AusdienstDat, Kunden.KdNr AS [KdNr letzter Kunde], Kunden.SuchCode AS [letzter Kunde]
FROM TeileLag
