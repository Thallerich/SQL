...
SELECT Firma.SuchCode AS Firma, Firma.Bez AS Firmenbezeichnung, Lager.SuchCode AS Lagerstandort, Lagerart.Lagerart, Lagerart.LagerartBez AS [Lagerart Bezeichnung], Artikel.ArtikelNr, Artikel.ArtikelBez AS Artikelbezeichnung, ArtGroe.Groesse AS Größe, Bestand.Bestand AS Lagerbestand, ArtGroe.EKPreis AS EKPreis, N'EUR' AS Währung, CAST(NULL AS money) AS [EKPreis SMZL], Artikel.ID AS ArtikelID, ArtGroe.ID AS ArtGroeID, Lager.ID AS StandortID, CAST(NULL AS nchar(10)) AS Updated
INTO #BestandGLD
FROM Bestand
...
SELECT Firma, Firmenbezeichnung, Lagerstandort, Lagerart, [Lagerart Bezeichnung], ArtikelNr, Artikelbezeichnung, Größe, Lagerbestand, EKPreis, Währung, [EKPreis SMZL]
FROM #BestandGLD;
