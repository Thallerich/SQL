...
FROM [SALADVPSQLC1A1.salres.com].Salesianer.dbo.Mitarbei
WHERE Mitarbei.MaNr LIKE N'SMP%'
  AND NOT EXISTS (
    SELECT MA.*
    FROM Salesianer_SMP.dbo.Mitarbei AS MA
    WHERE MA.UserName = Mitarbei.UserName
  );

GO

INSERT INTO Salesianer_SMP.dbo.SichtUsr (SichtbarID, Sehen, UserID, Special0, Special1, Special2, Special3, Special4, Special5, Special6, Special7, Special8, Special9)
...
FROM [SALADVPSQLC1A1.salres.com].Salesianer.dbo.SichtUsr
JOIN [SALADVPSQLC1A1.salres.com].Salesianer.dbo.Mitarbei ON SichtUsr.UserID = Mitarbei.ID
JOIN Salesianer_SMP.dbo.Mitarbei AS TMA ON TMA.UserName = Mitarbei.UserName
WHERE Mitarbei.MaNr LIKE N'SMP%'
  AND NOT EXISTS (
    SELECT SU.*
    FROM Salesianer_SMP.dbo.SichtUsr AS SU
...
INSERT INTO Salesianer_SMP.dbo.UsrInGrp (MitarbeiID, UserGrpID)
SELECT TMA.ID AS MitarbeiID, USRINGRP.UserGrpID
FROM [SALADVPSQLC1A1.salres.com].Salesianer.dbo.UsrInGrp
JOIN [SALADVPSQLC1A1.salres.com].Salesianer.dbo.Mitarbei ON UsrInGrp.MitarbeiID = Mitarbei.ID
JOIN Salesianer_SMP.dbo.Mitarbei AS TMA ON TMA.UserName = Mitarbei.UserName
WHERE Mitarbei.MaNr LIKE N'SMP%'
  AND NOT EXISTS (
    SELECT UIG.*
    FROM Salesianer_SMP.dbo.UsrInGrp AS UIG
    WHERE UIG.MitarbeiID = UsrInGrp.MitarbeiID
      AND UIG.UserGrpID = UsrInGrp.UserGrpID
  );
