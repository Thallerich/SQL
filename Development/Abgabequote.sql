...
FROM (
  SELECT Kunden.KdNr, Kunden.SuchCode AS Kunde, Artikel.ArtikelNr, Artikel.ArtikelBez AS Artikelbezeichnung, Bereich.BereichBez AS Produktbereich, ArtGru.ArtGruBez AS Artikelgruppe, ProdGru.ProdGruBez AS Sortiment, Wochen.Woche, SUM(TraeArch.Menge) AS Menge, SUM(TraeArch.Effektiv) AS Effektiv, IIF(SUM(TraeArch.Menge) < 3, CAST((SUM(TraeArch.Menge) / 3.0) AS numeric(18, 4)), CAST(((SUM(TraeArch.Menge) - 1) / 2.0) AS numeric(18, 4))) AS Ruecklauf, COUNT(DISTINCT Traeger.ID) AS TraeAnz
  FROM TraeArch
  ...
  GROUP BY Kunden.KdNr, Kunden.SuchCode, Artikel.ArtikelNr, Artikel.ArtikelBez, Bereich.BereichBez, ArtGru.ArtGruBez, ProdGru.ProdGruBez, Wochen.Woche
) AS Ruecklaufdaten
