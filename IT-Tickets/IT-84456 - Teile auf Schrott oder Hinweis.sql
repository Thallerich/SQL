...
CREATE NONCLUSTERED INDEX IX_Cleanup ON #Cleanup (EinzTeilID) WITH (DATA_COMPRESSION = PAGE);
...
WHERE EinzHist.Barcode IN (SELECT Barcode FROM _IT84456 WHERE Schrott = 1 AND Hinweis = 0)
...
DECLARE @hinweis nvarchar(128) = N'Bitte Aygül geben - Es muss nach Farbe und Waschkatagorie angelegt werden.';
DECLARE @hinweistextid int = 1000539; /* Kundenservice (SM) - 503 */
DECLARE @userid int = (SELECT ID FROM Mitarbei WHERE UserName = N'THALST')
DECLARE @arbplatzid int = (SELECT ID FROM ArbPlatz WHERE ComputerName = HOST_NAME());
DECLARE @msg nvarchar(max);

SELECT @msg = FORMAT(GETDATE(), N'dd.MM.yyyy HH:mm:ss', N'de-AT') + N': Teile für Hinweis-Anlage in Temp-Table laden';
RAISERROR(@msg, 0, 1) WITH NOWAIT;
...
JOIN EinzHist ON EinzTeil.CurrEinzHistID = EinzHist.ID
JOIN Kunden ON EinzHist.KundenID = Kunden.ID
WHERE EinzHist.Barcode IN (SELECT Barcode FROM _IT84456 WHERE Schrott = 0 AND Hinweis = 1)
  AND EinzHist.EinzHistTyp = 1;

SELECT @msg = FORMAT(GETDATE(), N'dd.MM.yyyy HH:mm:ss', N'de-AT') + N': Anzahl Teile für Hinweis-Anlage = ' + FORMAT(@@ROWCOUNT, N'N0', N'de-AT');
RAISERROR(@msg, 0, 1) WITH NOWAIT;

BEGIN TRY
  BEGIN TRANSACTION;
  
    INSERT INTO Hinweis (EinzHistID, Aktiv, Hinweis, BisWoche, Anzahl, EingabeDatum, HinwTextID, EingabeMitarbeiID, Wichtig, AnlageUserID_, UserID_)
    SELECT #Cleanup.EinzHistID, CAST(1 AS bit) AS Aktiv, @hinweis AS Hinweis, N'2099/52' AS BisWoche, 1 AS Anzahl, GETDATE() AS EingabeDatum, @hinweistextid AS HinwTextID, @userid AS EingabeMitarbeiID, 1 AS Wichtig, @userid AS AnlageUserID_, @userid AS UserID_
    FROM #Cleanup;

    SELECT @msg = FORMAT(GETDATE(), N'dd.MM.yyyy HH:mm:ss', N'de-AT') + N': Hinweis angelegt';
    RAISERROR(@msg, 0, 1) WITH NOWAIT;

    UPDATE EinzHist SET HasHinweis = 1
    WHERE EinzHist.ID IN (SELECT EinzHistID FROM #Cleanup)
      AND EinzHist.HasHinweis = 0;

    SELECT @msg = FORMAT(GETDATE(), N'dd.MM.yyyy HH:mm:ss', N'de-AT') + N': Hinweis-Kennzeichen auf EinzHist-Datensatz aktualisiert';
    RAISERROR(@msg, 0, 1) WITH NOWAIT;
