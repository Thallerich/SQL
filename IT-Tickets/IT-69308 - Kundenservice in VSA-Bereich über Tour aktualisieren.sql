...
CREATE TABLE __IT69308_ServiceID (
  VsaBerID int,
  ServiceiD int
);
...
JOIN Salesianer.dbo._IT69308 ON Touren.Tour = _IT69308.Tour AND Kunden.KdNr = _IT69308.KdNr
JOIN Mitarbei ON _IT69308.Kundenservice = Mitarbei.Name
...
INTO __IT69308_ServiceID (VsaBerID, ServiceiD)
...
FROM (
  ...
  JOIN Vsa ON VsaBer.VsaID = Vsa.ID
  JOIN Kunden ON Vsa.KundenID = Kunden.ID
  JOIN Salesianer.dbo._IT69308 ON Touren.Tour = _IT69308.Tour
  JOIN Mitarbei ON _IT69308.Kundenservice = Mitarbei.Name
  WHERE VsaTour.BisDatum > CAST(GETDATE() AS date)
    AND Vsa.[Status] = N'A'
    AND Kunden.[Status] = N'A'
