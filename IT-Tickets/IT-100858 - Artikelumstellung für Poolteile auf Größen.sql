...
DECLARE @kundenid int = (SELECT ID FROM Kunden WHERE KdNr = 10003771);
...
BEGIN
  RAISERROR('ArtikelIDs konnte nicht vollst�ndig ermittelt werden', 16, 1);
  SET NOEXEC ON;
END;
...
BEGIN TRY
  BEGIN TRANSACTION;
    ...
    SET @msg = FORMAT(GETDATE(), 'yyyy-MM-dd HH:mm:ss') + ' - ' + FORMAT(@@ROWCOUNT, N'N0') + ' Kundenartikel eingef�gt!';
    RAISERROR(@msg, 0, 1) WITH NOWAIT;
    ...
    SET @msg = FORMAT(GETDATE(), 'yyyy-MM-dd HH:mm:ss') + ' - ' + FORMAT(@@ROWCOUNT, N'N0') + ' Preisarchiv-Eintr�ge eingef�gt!';
    RAISERROR(@msg, 0, 1) WITH NOWAIT;
    ...
    SEt @msg = FORMAT(GETDATE(), 'yyyy-MM-dd HH:mm:ss') + ' - tempor�re Arbeitstabellen mit neu angelegten Kundenartikeln aktualisiert!';
    RAISERROR(@msg, 0, 1) WITH NOWAIT;
    ...
    WHERE AnfKo.Lieferdatum >= CAST(GETDATE() AS date)
      ...
      AND AnfKo.VsaID IN (SELECT Vsa.ID FROM Vsa WHERE Vsa.KundenID = @kundenid);
