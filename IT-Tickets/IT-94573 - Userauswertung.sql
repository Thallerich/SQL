SELECT Mitarbei.ID AS MitarbeiID,
  ...
  Mitarbei.Nachname,
  Mitarbei.LastLogin,
  [deaktivieren] = CAST(IIF((Mitarbei.LastLogin < N'2025-01-01 00:00:00' OR Mitarbei.LastLogin IS NULL) AND Mitarbei.Anlage_ < DATEADD(month, -1, GETDATE()) AND NOT EXISTS (SELECT UsrInGrp.* FROM UsrInGrp WHERE UsrInGrp.MitarbeiID = Mitarbei.ID AND UsrInGrp.UserGrpID IN (SELECT UserGrp.ID FROM UserGrp WHERE UserGrp.UserGrpBez IN (N'SAL_SDC', N'SAL_ADVSUP_only'))), 1, 0) AS bit),
