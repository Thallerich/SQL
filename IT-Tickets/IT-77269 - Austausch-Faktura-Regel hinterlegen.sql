...
BEGIN TRY
  BEGIN TRANSACTION;
    ...
    INSERT INTO KdTauFak (KundenID, MaxAnzWochen, AnlageUserID_, UserID_)
    SELECT CustomerID AS KundenID, CAST(104 AS int) AS MaxWochen, @UserID AS AnlageUserID_, @UserID AS UserID_
    FROM @Customer
    WHERE NOT EXISTS (
      SELECT KdTauFak.*
      FROM KdTauFak
